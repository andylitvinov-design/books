...
const blankItem = () => ({ remedySlug: '', displayNameOverride: '', potency: '', purpose: '', sequence: '', dosage: '', frequency: '', duration: '', instructions: '', notes: '', query: '' })
...
export function PrescriptionForm({ action, prescription, remedies }) {
  ...
  const serializedItems = useMemo(() => JSON.stringify(items.map((item) => { const stored = { ...item }; delete stored.query; return stored })), [items])
  ...
  return (
    <form className="prescription-admin-form" action={action}>
      ...
      <fieldset><legend>Remedies</legend>
        {items.map((item, index) => {
          ...
          return <section className="prescription-admin-item" key={index}>
            <div><strong>Item {index + 1}</strong><button type="button" onClick={() => { setItems((current) => current.filter((_, itemIndex) => itemIndex !== index)); setQueries((current) => current.filter((_, itemIndex) => itemIndex !== index)) }} disabled={items.length === 1}>Remove</button></div>
            ...
            <div className="prescription-admin-item-fields">
              {['potency', 'purpose', 'dosage', 'frequency', 'duration', 'sequence', 'instructions'].map((field) => <label key={field}>{field}<input value={item[field]} onChange={(event) => update(index, { [field]: event.target.value })} /></label>)}
            </div>
          </section>
        })}
      ...
      <label>Follow-up<textarea name="followUp" defaultValue={prescription?.followUp} /></label>
      ...
      <div className="prescription-admin-actions">
        ...
        <button type="submit" name="status" value="active">Save recommendation</button>
