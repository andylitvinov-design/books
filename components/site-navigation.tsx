...
export function SiteNavigation({ locale, onLocaleChange }: SiteNavigationProps) {
  ...
  const labels = activeLocale === "ru"
    ? { home: "Главная", book: "Книга", remedies: "Препараты", services: "Услуги", about: "Обо мне", events: "События", cabinet: "Кабинет" }
    : { home: "Home", book: "Book", remedies: "Remedies", services: "Services", about: "About", events: "Events", cabinet: "Cabinet" };
  ...
  return (
    <nav aria-label={activeLocale === "ru" ? "Основная навигация" : "Primary navigation"} className="site-navigation">
      ...
      <Link href={"/" + activeLocale + "/about#events"}>{labels.events}</Link>
