...
export default async function NewConsultation({ searchParams }) {
  ...
  return <main className="prescription-admin-shell consultation-new-shell"><PrescriptionAdminHeader title={{ ru: "Новая консультация", en: "New consultation" }} description={{ ru: "Клиент → назначение → документы.", en: "Client → recommendation → documents." }} /><ConsultationForm clients={options} selectedClientId={(await searchParams).clientId} action={createConsultationAction} remedies={getConsultationRemedyOptions()} requestId={randomUUID()} /></main>
}
