...
test('Book 02 TOC is an alphabetical projection of all 95 published canonical cards', () => {
  ...
  assert.equal(toc.remedy_count, 95)
  assert.equal(toc.entries.length, 95)
  assert.equal(new Set(toc.entries.map(({ slug }) => slug)).size, 95)
