...
function localized(locale) {
  return locale === 'ru'
    ? { title: 'ГОМЕОПАТИЧЕСКОЕ НАЗНАЧЕНИЕ / РЕКОМЕНДАЦИЯ', continued: 'Продолжение', patient: 'Клиент', date: 'Дата', dob: 'Дата рождения', remedy: 'Препарат', potency: 'Разведение', dosage: 'Приём', frequency: 'Частота', duration: 'Срок', instructions: 'Рекомендации', updated: 'Обновлено', disclaimer: 'Материал носит информационный характер и не заменяет медицинскую диагностику, лечение или неотложную медицинскую помощь.' }
    : { title: 'HOMEOPATHIC RECOMMENDATION', continued: 'Continued', patient: 'Patient', date: 'Date', dob: 'Date of birth', remedy: 'Remedy', potency: 'Potency', dosage: 'Dosage', frequency: 'Frequency', duration: 'Duration', instructions: 'Recommendations', updated: 'Updated', disclaimer: 'This document is provided for informational purposes and is not a substitute for medical diagnosis, treatment, or emergency care.' }
}
...
export function buildPrescriptionPdf(document, locale, origin) {
  ...
  const layouts = []
  const contentBottom = 140
  let page
  let y

  function startPage(continuation = false) {
    page = { commands: ['0.15 0.17 0.2 rg'], links: [] }
    layouts.push(page)
    y = A4.top
    if (continuation) {
      text(page.commands, `${copy.title} — ${copy.continued}`, A4.left, y, 12, unicode)
      y -= 25
    }
  }

  function ensureSpace(height = 12) {
    if (y - height < contentBottom) startPage(true)
  }

  function line(value, x, size, height) {
    ensureSpace(height)
    text(page.commands, value, x, y, size, unicode)
    y -= height
  }

  function itemHeading(item) {
    ensureSpace(30)
    text(page.commands, `${copy.remedy}: ${item.displayName}`, A4.left, y, 11, unicode)
    if (item.remedyPath) {
      page.links.push({ url: new URL(item.remedyPath, origin).toString(), y })
      page.commands.push(`0.36 0.18 0.1 RG ${A4.left} ${y - 2} m 310 ${y - 2} l S`, '0.15 0.17 0.2 rg')
    }
    y -= 14
  }

  startPage()
  line(document.practitionerName, A4.left, 16, 22)
  for (const value of [document.practitionerRole, document.practitionerBackground, document.practitionerContact].filter(Boolean)) line(value, A4.left, 9, 13)
  y -= 16
  line(copy.title, A4.left, 15, 28)
  line(`${copy.patient}: ${document.patientName}`, A4.left, 10, 15)
  line(`${copy.date}: ${document.dateIssued}`, A4.left, 10, 15)
  if (document.patientDob) line(`${copy.dob}: ${document.patientDob}`, A4.left, 10, 15)
  y -= 10

  for (const item of document.items) {
    itemHeading(item)
    for (const detail of prescriptionItemLines(item, copy)) {
      for (const detailLine of wrap(detail, 74, Boolean(unicode))) line(detailLine, A4.left + 12, 9, 12)
    }
    y -= 8
  }
  ...
  if (document.generalInstructions) {
    ensureSpace(26)
    text(page.commands, copy.instructions, A4.left, y, 11, unicode)
    y -= 14
    for (const instructionLine of wrap(document.generalInstructions, 78, Boolean(unicode))) line(instructionLine, A4.left, 9, 12)
  }

  for (const layout of layouts) {
    const footerY = A4.bottom + 34
    text(layout.commands, `${copy.updated}: ${document.updatedAt.slice(0, 10)}`, A4.left, footerY + 24, 8, unicode)
    text(layout.commands, document.practitionerName, A4.left, footerY + 10, 9, unicode)
    layout.commands.push(`0.35 0.35 0.35 RG ${A4.left} ${footerY} m 232 ${footerY} l S`)
    for (const [index, disclaimerLine] of wrap(copy.disclaimer, 98, Boolean(unicode)).entries()) text(layout.commands, disclaimerLine, A4.left, footerY - 16 - (index * 10), 7, unicode)
  }
  ...
  const pageIds = layouts.map((layout) => {
    ...
    const annotations = layout.links.map(({ url, y: linkY }) => pdfObject(objects, `<< /Type /Annot /Subtype /Link /Rect [${A4.left} ${linkY - 3} 310 ${linkY + 12}] /Border [0 0 0] /A << /S /URI /URI (${literal(url)}) >> >>`))
    return pdfObject(objects, `<< /Type /Page /Parent ${pages} 0 R /MediaBox [0 0 ${A4.width} ${A4.height}] /Resources << /Font <<${fonts} >> >> /Contents ${content} 0 R${annotations.length ? ` /Annots [${annotations.map((id) => `${id} 0 R`).join(' ')}]` : ''} >>`)
  })
