...
function rateDigest(scope, value) {
  return createHash('sha256').update(`${scope}\0${String(value).slice(0, 128)}`, 'utf8').digest('hex')
}
...
export async function exchangePrescriptionAccess({ request, store, ip = 'unknown', nowMs = Date.now() }) {
  ...
  if (!isSelector(body.selector) || !isBearerSecret(body.secret)) return prescriptionAccessFailure
  const ipAllowed = await store.consumeAccessAttempt(rateDigest('ip', ip), 30, 5 * 60)
  if (!ipAllowed) return prescriptionAccessFailure
  const selectorAllowed = await store.consumeAccessAttempt(rateDigest('selector', body.selector), 12, 5 * 60)
  if (!selectorAllowed) return prescriptionAccessFailure
...
export async function closePrescriptionSession({ cookieStore, store, name = prescriptionSessionCookieName() }) {
  const token = cookieStore.get(name)?.value
  cookieStore.delete(name)
  const digest = digestSessionToken(token)
  if (!store || !digest) return
  try { await store.deleteAccessSession(digest) } catch { /* cookie expiry remains authoritative in this browser */ }
}

export async function deletePrescriptionRequestSession() {
  const cookieStore = await cookies()
  await closePrescriptionSession({ cookieStore, store: getPrescriptionStore() })
}
