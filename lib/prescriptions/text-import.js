...
const fieldNames = {
  ...
  instructions: /^(?:instructions?|инструкции|notes?|заметки|примечани[ея])$/i,
}
...
function parseDetails(text, item) {
  ...
  rest = rest.replace(/(?:^|[,;|]\s*|\s+)(potency|потенция|разведение|dose|dosage|доза|дозировка|frequency|частота|при[её]м|duration|длительность|курс|instructions?|инструкции|notes?|заметки|примечани[ея])\s*:\s*([\s\S]*?)(?=(?:[,;|]\s*|\s+)(?:potency|потенция|разведение|dose|dosage|доза|дозировка|frequency|частота|при[её]м|duration|длительность|курс|instructions?|инструкции|notes?|заметки|примечани[ея])\s*:|$)/gi, (_, label, value) => {
  ...
  if (remaining) item.instructions = [item.instructions, remaining].filter(Boolean).join('; ')
  return remaining
}
...
export function parsePrescriptionText(source, remedies) {
  ...
  for (const [index, raw] of source.split(/\r?\n/).entries()) {
    ...
    if (cells && /^(?:remedy|препарат|средство)$/i.test(cells[0])) {
      tableColumns = cells.map((cell, i) => i === 0 ? 'remedy' : Object.keys(fieldNames).find((key) => fieldNames[key].test(cell)) ?? 'instructions')
      continue
    }
    ...
    if (match) {
      ...
      let residual = ''
      if (cells) {
        const mapping = tableColumns ?? ['remedy', 'potency', 'dosage', 'frequency', 'duration', 'instructions']
        cells.slice(1).forEach((cell, i) => { const key = mapping[i + 1] ?? 'instructions'; item[key] = [item[key], cell].filter(Boolean).join('; ') })
        if (match.rest) residual = parseDetails(match.rest, item)
      } else residual = parseDetails(match.rest, item)
      if (residual) result.warnings.push(`Строка ${index + 1}: проверьте остаток текста в Instructions — ${residual}`)
