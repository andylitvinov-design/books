export function initialPrescriptionRows(prescription, remedies, count = 4) {
  ...
  return prescription?.items?.map((item) => ({ ...blank(), ...item, query: (item.remedySlug ? remedies.find((remedy) => remedy.slug === item.remedySlug)?.label : null) ?? item.displayNameOverride ?? item.remedySlug ?? '' })) ?? Array.from({ length: count }, blank)
}
