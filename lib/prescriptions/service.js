...
export const recommendationTypes = ['homeopathy', 'bach']
...
function validateItem(value, index) {
  const remedy = resolveRecommendationRemedy(value, index)
  return {
    id: randomUUID(),
    sortOrder: index,
    ...remedy,
    ...
    granules: optionalDoseCount(value?.granules, 'Granules'),
    timesPerDay: optionalDoseCount(value?.timesPerDay, 'Times per day'),
...
export function validatePrescriptionInput(input) {
  ...
  const recommendationType = input.recommendationType ?? 'homeopathy'
  if (!recommendationTypes.includes(recommendationType)) throw new Error('Invalid recommendation type')
...
export function getClientPrescription(record, locale) {
  ...
  return {
    ...
    recommendationType: record.recommendationType ?? 'homeopathy',
