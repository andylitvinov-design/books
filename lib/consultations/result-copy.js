...
const copy = {
  en: {
    ...
    payment: 'PAYMENT DOCUMENT', recommendation: 'HOMEOPATHIC RECOMMENDATION', bachRecommendation: 'BACH FLOWER ESSENCE RECOMMENDATION', receipt: 'Receipt', invoice: 'Invoice',
  ...
  ru: {
    ...
    payment: 'ПЛАТЁЖНЫЙ ДОКУМЕНТ', recommendation: 'ГОМЕОПАТИЧЕСКАЯ РЕКОМЕНДАЦИЯ', bachRecommendation: 'РЕКОМЕНДАЦИЯ ПО ЭССЕНЦИЯМ БАХА', receipt: 'Квитанция', invoice: 'Счёт',
...
export function resultDocumentText(document, locale) {
  ...
  const title = document.recommendationType === 'bach' ? labels.bachRecommendation : labels.recommendation
  const description = document.recommendationType === 'bach'
    ? language === 'ru' ? `${count} ${count === 1 ? 'эссенция' : count >= 2 && count <= 4 ? 'эссенции' : 'эссенций'}` : `${count} ${count === 1 ? 'essence' : 'essences'}`
