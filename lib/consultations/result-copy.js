...
const copy = {
  en: {
    ...
    payment: 'PAYMENT DOCUMENT', recommendation: 'HOMEOPATHIC RECOMMENDATION', receipt: 'Receipt', invoice: 'Invoice',
  ...
  ru: {
    ...
    payment: 'ПЛАТЁЖНЫЙ ДОКУМЕНТ', recommendation: 'ГОМЕОПАТИЧЕСКАЯ РЕКОМЕНДАЦИЯ', receipt: 'Квитанция', invoice: 'Счёт',
...
export function resultDocumentText(document, locale) {
  ...
  return { title: labels.recommendation, edit: labels.editRecommendation, description: language === 'ru' ? `${count} ${word}` : remedyCountLabel(count) }
}
