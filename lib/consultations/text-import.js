...
function homeopathyItem(item) {
  ...
  let instructions = String(item.instructions ?? '').trim()
  let potency = normalizePotency(item.potency)
  if (!item.potency) {
    const plain = instructions.match(/^(?:потенция\s*[:=]?\s*)?(\d{1,4})(?:\s*[,;.-]\s*|$)/i)
    if (plain) {
      potency = plain[1]
      instructions = instructions.slice(plain[0].length).trim()
    }
  }
  return {
    ...item,
    itemType: 'homeopathy',
    selected: true,
    displayNameOverride: item.remedySlug ? null : item.displayNameOverride || item.query,
    sourceStatus: item.remedySlug ? 'canonical' : 'source_only',
    potency,
    granules: granules || '5',
