...
function documentInputs(input) {
  if (!Array.isArray(input.items) || input.items.some((item) => !item?.remedySlug || item.displayNameOverride)) {
    throw new Error('Every item requires a canonical remedy slug without a name override')
  }
