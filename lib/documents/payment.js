...
export function validatePaymentDocumentInput(input, now = new Date().toISOString()) {
  ...
  if (paymentMethod && paymentMethod.length > 120) throw new Error('Payment method must be at most 120 characters')
  ...
  return {
    ...
    consultations: count ? Number(count) : undefined, paymentMethod,
...
export function updatePaymentDocument(existing, input, now = new Date().toISOString()) {
  ...
  if (existing.clientId) updated.clientId = existing.clientId
  if (existing.consultationId) updated.consultationId = existing.consultationId
...
export function getClientPaymentDocument(record, locale) {
  ...
  return {
    ...
    consultations: record.consultations, documentNumber: record.documentNumber, paymentMethod: record.paymentMethod,
