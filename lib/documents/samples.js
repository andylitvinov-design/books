...
export function getDocumentSample(kind, locale = 'en') {
  ...
  return getClientPrescription(createPrescription({ patientName, practitionerName: 'Andrii Litvinov', dateIssued: '2026-09-08', recommendationNumber: 'HR-TEST-001', status: 'active', items: [{ remedySlug: 'arsenicum-album', potency: 'TEST potency', purpose: 'Synthetic practitioner-entered purpose', dosage: 'TEST dosage - not clinical guidance', frequency: 'TEST frequency', duration: 'TEST duration', sequence: 'TEST stage', instructions: 'Synthetic individual instructions.' }, { remedySlug: 'aurum-metallicum', instructions: 'Synthetic second item - not clinical guidance.' }], generalInstructions: locale === 'ru' ? 'Тестовые рекомендации. Только синтетический образец.' : 'Synthetic practitioner-entered general recommendations.', followUp: 'Synthetic follow-up guidance.' }, now), locale)
}
