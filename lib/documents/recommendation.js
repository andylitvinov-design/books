const shared = {
  ru: {
    ...
    general: 'ДОПОЛНИТЕЛЬНЫЕ РЕКОМЕНДАЦИИ',
...
export function normalizeRecommendationType(value) {
  return value === 'bach' ? 'bach' : 'homeopathy'
}
...
function sameValue(items, key) {
  const values = (items ?? []).map((item) => String(item?.[key] ?? '').trim()).filter(Boolean)
...
export function recommendationGuidance(document, locale = 'en') {
  ...
  const granules = sameValue(document.items, 'granules')
  const timesPerDay = sameValue(document.items, 'timesPerDay')
  if (language === 'ru') {
    const granuleLine = granules
      ? `${granules} ${Number(granules) === 1 ? 'гранула препарата' : 'гранул препарата'}.`
      : 'Дозировка указана рядом с каждым препаратом.'
    const frequencyLine = timesPerDay
      ? `${russianTimes(timesPerDay)} в день и дополнительно в момент стресса.`
      : 'Частота приёма указана рядом с каждым препаратом; дополнительно — в момент стресса.'
    return {
      takeTitle: 'ПРИНИМАТЬ',
      bullets: [granuleLine, frequencyLine],
      course: 'Курс: 2 недели.',
      recheck: 'Повторная проверка — через 1–2 недели.',
      contact: 'Мне можно написать по готовности — я проверю состояние и дам анализ, рекомендации или при необходимости скорректирую назначение.',
    }
  ...
  const granuleLine = granules
    ? `${granules} ${Number(granules) === 1 ? 'granule' : 'granules'} of the remedy.`
    : 'Use the dose shown next to each remedy.'
  const frequencyLine = timesPerDay
    ? `${timesPerDay} times a day and additionally during moments of stress.`
    : 'Use the frequency shown next to each remedy and additionally during moments of stress.'
  return {
    takeTitle: 'HOW TO TAKE',
    bullets: [granuleLine, frequencyLine],
    course: 'Course: 2 weeks.',
...
export function remedySchedule(item, locale = 'en', recommendationType = 'homeopathy') {
  if (normalizeRecommendationType(recommendationType) === 'bach') return ''
